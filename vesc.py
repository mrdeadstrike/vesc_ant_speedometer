...
while running:
  ...
  battery_rect = battery_text.get_rect(center=(WIDTH//2 - 40, 700))
  screen.blit(battery_text, battery_rect)

  battery_color = get_battery_color(data['battery_level'])
  draw_progress_bar(screen, battery_rect.right + 10, 700 - 10, 100, 20, data['battery_level'], 100, battery_color)

  # 5. Одометр
  draw_text_center(screen, f"{data['odometer']} км", font_small, (200, 200, 0), 760)
  if trip_start_time is not None:
    trip_time = time.time() - trip_start_time
    minutes = int(trip_time // 60)
    seconds = int(trip_time % 60)
    draw_text(screen, f"{minutes:02d}:{seconds:02d}", font_small, (255, 255, 255), 400, 760)
  ...
  date_rect = date_text.get_rect(topleft=(20, 740 - 5))
